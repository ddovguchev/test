...
export function getModeSize(gdkmonitor: Gdk.Monitor, mode: string): { width: number; height: number } {
    const geometry = (gdkmonitor as Gdk.Monitor & { get_geometry?: () => { width?: number } })?.get_geometry?.()
